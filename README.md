...
cd server
npm install
cp ../.env.example .env    # then fill in real values in server/.env
...
No build step — it's static files. Serve `frontend/` with any static file
server. If you use VS Code's Live Server extension on its default port, it'll
match the default `FRONTEND_URL` already set for local dev
(`http://localhost:5500`) and everything works without extra config.
...
`http://localhost:5000/api` automatically — no local configuration needed.

---

## Environment Variables

See `.env.example` for the full list with explanations. Required:
