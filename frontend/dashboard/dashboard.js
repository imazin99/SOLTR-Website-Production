  ...
  try {
    ...
    tbody.innerHTML = orders.map(o => {
      const itemsSummary = (o.items || [])
        .map(i => `${i.name} (${i.color || '?'}) ×${i.quantity}`).join(', ');
      ...
      return `
        <tr data-order-id="${o._id}">
          ...
          <td style="font-size:11px;color:var(--smoke);max-width:160px;white-space:normal;line-height:1.4;">${itemsSummary || '—'}</td>
          <td style="font-family:var(--mono);font-size:12px;">${fmt(o.total)}</td>
          <td>${statusBadge(o.status)}</td>
          <td><span class="badge ${PAYMENT_BADGE[o.paymentStatus] || ''}">${o.paymentStatus || '—'}</span></td>
          <td style="font-family:var(--mono);font-size:11px;color:var(--smoke);white-space:nowrap;">${dateStr}</td>
          <td>
...
async function renderCustomers(query) {
  ...
  tbody.innerHTML = list.map(c => `
    <tr>
      ...
      <td class="cust-email">${c.email || '—'}</td>
      <td style="font-family:var(--mono);font-size:11px;color:var(--smoke);">${c.phone}</td>
      <td style="font-family:var(--mono);font-size:12px;text-align:center;">${c.totalOrders}</td>
      <td style="font-family:var(--mono);font-size:12px;">${fmt(c.totalSpent)}</td>
      <td style="font-family:var(--mono);font-size:11px;color:var(--smoke);">${new Date(c.joinedAt).toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' })}</td>
    </tr>`).join("");
}
...
async function renderCoupons() {
  ...
  try {
    ...
    tbody.innerHTML = coupons.map(c => {
      ...
      return `
        <tr>
          <td style="font-family:var(--mono);font-size:13px;letter-spacing:.06em;">${c.code}</td>
          <td style="font-family:var(--mono);font-size:13px;">${discountLabel}</td>
          <td style="font-family:var(--mono);font-size:12px;color:var(--smoke);">${c.minimumOrderAmount > 0 ? fmt(c.minimumOrderAmount) : '—'}</td>
          <td style="font-family:var(--mono);font-size:11px;color:var(--smoke);">${fmtDate(c.expiryDate)}</td>
          <td style="font-family:var(--mono);font-size:12px;">${c.usedCount}${c.usageLimit > 0 ? ' / ' + c.usageLimit : ''}</td>
          <td style="font-family:var(--mono);font-size:12px;color:var(--smoke);">${remaining}</td>
