...
function imgUrl(f) { return window.productImageUrl(f); }

/** Escape server-provided text before inserting it into dashboard HTML. */
...
async function renderRecentOrders() {
  ...
  try {
    ...
    tbody.innerHTML = orders.map(o => `
      <tr>
        <td class="order-id">${esc(o.orderNumber)}</td>
        <td>${esc(o.customer?.name)}</td>
        <td><span style="font-family:var(--mono);font-size:12px;">${esc(fmt(o.total))}</span></td>
        <td>${statusBadge(esc(o.status))}</td>
      </tr>`).join('');
  } catch {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--smoke);padding:20px;
  ...
  try {
    ...
    list.innerHTML = products.map((p, i) => `
      <div class="top-product-row">
        ...
        <img class="tp-img" src="${esc(imgUrl(p.images && p.images[0]))}" alt="${esc(p.name)}" loading="lazy"
             onerror="handleProductImageError(this)">
        <div class="tp-info">
          <div class="tp-name">${esc(p.name)}</div>
          <div class="tp-meta">${esc((p.colors || []).join(', '))} · ${esc(p.category)}</div>
        </div>
        <div class="tp-sales">${esc(fmt(p.price))}</div>
      </div>`).join("");
  } catch {
  ...
  try {
    ...
    tbody.innerHTML = orders.map(o => {
      const itemsSummary = (o.items || [])
        .map(i => `${esc(i.name)} · ${esc(i.size || '—')} ×${esc(i.quantity)}`).join(', ');
      const dateStr = new Date(o.createdAt).toLocaleDateString('en-GB',
        { day:'numeric', month:'short', year:'numeric' });
      const statusOpts = STATUS_OPTIONS
        .map(s => `<option value="${esc(s)}" ${o.status === s ? 'selected' : ''}>${esc(s)}</option>`)
        .join('');

      return `
        <tr data-order-id="${esc(o._id)}">
          <td class="order-id" data-label="Order">${esc(o.orderNumber)}</td>
          <td data-label="Customer">
            <div class="customer-cell">
              <div class="cust-av">${esc(initials(o.customer?.name))}</div>
              <div>
                <div class="cust-name">${esc(o.customer?.name)}</div>
                <div class="cust-email">${esc(o.customer?.phone)}</div>
                ${o.customer?.city ? `<div class="cust-email" style="font-size:9.5px;">${esc(o.customer.city)}</div>` : ''}
              </div>
            </div>
          </td>
          ...
          <td data-label="Total" style="font-family:var(--mono);font-size:12px;">${esc(fmt(o.total))}</td>
          <td data-label="Status">${statusBadge(o.status)}</td>
          <td data-label="Payment"><span class="badge ${PAYMENT_BADGE[o.paymentStatus] || ''}">${esc(o.paymentStatus || '—')}</span></td>
          ...
          <td data-label="Actions">
            <div class="order-actions-cell">
              <select class="status-select" onchange="quickUpdateStatus(${esc(JSON.stringify(o._id))}, this.value)" title="Quick status">
                ${statusOpts}
              </select>
              <button class="action-btn" onclick="openOrderDetail(${esc(JSON.stringify(o._id))})">View</button>
              <button class="action-btn" onclick="openOrderModal(${esc(JSON.stringify(o._id))})">Edit</button>
              <button class="action-btn del-btn" onclick="deleteOrderConfirm(${esc(JSON.stringify(o._id))},${esc(JSON.stringify(o.orderNumber))})">Del</button>
            </div>
          </td>
        </tr>`;
    }).join('');
    ...
    grid.innerHTML = products.map(p => {
      ...
      const sizeBreakdown = sizeInventory
        ? (p.sizes || []).map(size => {
            ...
            return `<span class="pa-size-chip${qty === 0 ? ' is-empty' : ''}">${esc(size)}<b>${esc(qty)}</b></span>`;
          }).join('')
        : '';
      return `
        <div class="product-admin-card" data-id="${esc(p._id)}">
          <div class="pa-img"><img src="${esc(src)}" alt="${esc(p.name)}" loading="lazy" onerror="handleProductImageError(this)"></div>
          <div class="pa-info">
            <div class="pa-name">${esc(p.name)}</div>
            <div class="pa-meta">${esc(colors)} · ${esc(cat)}</div>
            <div class="pa-stock">
              <span class="pa-stock-qty">Stock: ${esc(stock)}</span>
              ${stockBadge(stock)}
            </div>
            ${sizeBreakdown ? `<div class="pa-size-breakdown" aria-label="Inventory by size">${sizeBreakdown}</div>` : ''}
            <div class="pa-foot">
                  <span class="pa-price">${esc(fmt(p.price))}</span>
              <div style="display:flex;gap:6px;">
                <button class="action-btn" onclick="openProductModal(${esc(JSON.stringify(p._id))})">Edit</button>
                <button class="action-btn del-btn" onclick="deleteProduct(${esc(JSON.stringify(p._id))})">Delete</button>
              </div>
            </div>
          </div>
        </div>`;
            ...
            <div class="existing-img-row">
              ${p.images.map(f => `
                <div class="existing-img-item">
                  <img src="${esc(imgUrl(f))}" alt="" onerror="handleProductImageError(this)">
...
async function renderCustomers(query) {
  ...
  tbody.innerHTML = list.map(c => `
    <tr>
      <td data-label="Customer">
        <div class="customer-cell">
          <div class="cust-av">${esc(initials(c.name))}</div>
          <div class="cust-name">${esc(c.name)}</div>
        </div>
      </td>
      <td data-label="Email" class="cust-email">${esc(c.email || '—')}</td>
      <td data-label="Phone" style="font-family:var(--mono);font-size:11px;color:var(--smoke);">${esc(c.phone)}</td>
      <td data-label="Orders" style="font-family:var(--mono);font-size:12px;text-align:center;">${esc(c.totalOrders)}</td>
      <td data-label="Total Spent" style="font-family:var(--mono);font-size:12px;">${esc(fmt(c.totalSpent))}</td>
...
async function renderCoupons() {
  ...
  try {
    ...
    tbody.innerHTML = coupons.map(c => {
      ...
      return `
        <tr>
          <td data-label="Code" style="font-family:var(--mono);font-size:13px;letter-spacing:.06em;">${esc(c.code)}</td>
          <td data-label="Discount" style="font-family:var(--mono);font-size:13px;">${esc(discountLabel)}</td>
          <td data-label="Min. Order" style="font-family:var(--mono);font-size:12px;color:var(--smoke);">${esc(c.minimumOrderAmount > 0 ? fmt(c.minimumOrderAmount) : '—')}</td>
          <td data-label="Expires" style="font-family:var(--mono);font-size:11px;color:var(--smoke);">${esc(fmtDate(c.expiryDate))}</td>
          <td data-label="Used" style="font-family:var(--mono);font-size:12px;">${esc(c.usedCount)}${c.usageLimit > 0 ? ' / ' + esc(c.usageLimit) : ''}</td>
          <td data-label="Remaining" style="font-family:var(--mono);font-size:12px;color:var(--smoke);">${esc(remaining)}</td>
          <td data-label="Status"><span class="badge ${statusClass}">${esc(statusLabel)}</span></td>
          <td data-label="Actions">
            <div style="display:flex;gap:6px;flex-wrap:wrap;">
              <button class="action-btn" onclick="openCouponModal(${esc(JSON.stringify(c._id))})">Edit</button>
              <button class="action-btn" onclick="toggleCouponActive(${esc(JSON.stringify(c._id))}, ${Boolean(c.active)})">${c.active ? 'Deactivate' : 'Activate'}</button>
              <button class="action-btn del-btn" onclick="deleteCoupon(${esc(JSON.stringify(c._id))}, ${esc(JSON.stringify(c.code))})">Delete</button>
            </div>
          </td>
        </tr>`;
    }).join("");
...
async function renderReviews() {
  ...
  try {
    ...
    tbody.innerHTML = reviews.map(r => {
      ...
      const reviewIdArg = esc(JSON.stringify(r._id));
      const customerNameArg = esc(JSON.stringify(r.customerName));

      let actions = '';
      if (reviewStatus === 'pending') {
        actions = `
          <button class="action-btn" onclick="approveReview(${reviewIdArg})">Approve</button>
          <button class="action-btn del-btn" onclick="rejectReview(${reviewIdArg}, ${customerNameArg})">Reject</button>`;
      } else {
        actions = `<button class="action-btn del-btn" onclick="deleteReview(${reviewIdArg}, ${customerNameArg})">Delete</button>`;
      }

      return `
        <tr>
          <td data-label="Reviewer" style="font-size:13px;">${esc(r.customerName)}</td>
          <td data-label="Rating" style="font-family:var(--mono);font-size:12px;color:var(--burgundy-light);">${esc('★'.repeat(r.rating))}${esc('☆'.repeat(5 - r.rating))}</td>
          <td data-label="Product" style="font-family:var(--mono);font-size:12px;color:var(--smoke);">${esc(r.productName)}</td>
          <td data-label="Review" style="font-size:12.5px;color:var(--smoke);max-width:260px;">${esc(snippet)}</td>
          <td data-label="Status"><span class="badge ${statusBadgeClass[reviewStatus] || ''}">${esc(reviewStatus.toUpperCase())}</span></td>
  ...
  container.innerHTML = data.map(d => {
    ...
    return `
      <div class="bar-col">
        ...
        <div class="bar-fill" style="height:${esc(pct)}%"></div>
        <div class="bar-label">${esc(d[labelKey])}</div>
      </div>`;
  }).join("");
}
...
function renderHorizontalChart(containerId, data) {
  ...
  container.innerHTML = data.map(d => `
    <div class="bar-col">
      <div class="bar-label">${esc(d.size)}</div>
      <div class="bar-fill" style="--pct:${esc(d.pct)}%; width:${esc(d.pct)}%"></div>
      <div class="bar-val">${esc(d.pct)}%</div>
    </div>`).join("");
}
...
function renderColorChart(data) {
  ...
  container.innerHTML = data.map(c => {
    ...
    return `
    <div class="donut-row">
      <div class="donut-swatch" style="background:${esc(hex)}; border:1px solid var(--line);"></div>
      <div class="donut-label">${esc(c.color)}</div>
      <div class="donut-bar-wrap">
        <div class="donut-bar" style="width:${esc(c.pct)}%; background:${esc(hex === "#1a1a1a" ? "var(--bone)" : hex)};"></div>
      </div>
      <div class="donut-pct">${esc(c.pct)}%</div>
    </div>`;
  }).join("");
}
...
async function renderPolicyList() {
  ...
  try {
    ...
    tbody.innerHTML = policies.map(p => `
      <tr>
        <td>${esc(p.title)}</td>
        <td style="font-family:var(--mono);font-size:12px;color:var(--smoke);">${esc(p.slug)}</td>
        <td style="font-family:var(--mono);font-size:11px;color:var(--smoke);">${fmtDate(p.updatedAt)}</td>
        <td><button class="action-btn" onclick="openPolicyEditor(${esc(JSON.stringify(p.slug))})">Edit</button></td>
      </tr>`).join('');
  } catch (err) {
...
function odInfoRow(label, value) {
  return `<div class="od-row"><span class="od-row-lbl">${esc(label)}</span><span class="od-row-val">${value}</span></div>`;
}
...
function renderOrderDetail(order) {
  ...
  document.getElementById('odMetaRow').innerHTML = `
    ...
    <span class="od-dot">·</span>
    <span>${esc(order.source || 'storefront')}</span>`;

  /* ── Customer info ── */
  document.getElementById('odCustomer').innerHTML = [
    odInfoRow('Name', esc(c.name || '—')),
    odInfoRow('Phone', c.phone
      ? `<a href="tel:${esc(encodeURIComponent(c.phone))}" style="color:var(--bone);text-decoration:none;">${esc(c.phone)}</a>`
      : '—'),
    odInfoRow('Email', c.email
      ? `<a href="mailto:${esc(encodeURIComponent(c.email))}" style="color:var(--bone);text-decoration:none;">${esc(c.email)}</a>`
      : '—'),
  ].join('');
  ...
  document.getElementById('odAddress').innerHTML = hasAddr
    ? [
        c.city    ? odInfoRow('City',    esc(c.city))    : '',
        c.address ? odInfoRow('Address', esc(c.address)) : '',
      ].join('')
  ...
  document.getElementById('odItems').innerHTML = (order.items || []).map(item => {
    ...
    return `
      <div class="od-item">
        <img class="od-item-img" src="${esc(src)}" alt="${esc(item.name || '')}"
             onerror="handleProductImageError(this)">
        <div class="od-item-body">
          <div class="od-item-name">${esc(item.name || '—')}</div>
          <div class="od-item-attrs">
            ${item.color ? `<span class="od-attr">Color: ${esc(item.color)}</span>` : ''}
            ${item.size  ? `<span class="od-attr">Size: ${esc(item.size)}</span>`  : ''}
            <span class="od-attr">Qty: ${esc(item.quantity || 1)}</span>
          </div>
        </div>
        <div class="od-item-price">${esc(fmt((item.price || 0) * (item.quantity || 1)))}</div>
      </div>`;
  }).join('');
  ...
  document.getElementById('odSummary').innerHTML = [
    ...
    odInfoRow('Payment Method', esc(order.paymentMethod || '—')),
