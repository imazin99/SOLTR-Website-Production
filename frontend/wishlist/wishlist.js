...
function renderWishlistPage() {
  ...
  grid.innerHTML = items.map(p => {
    ...
    const stock = p.sizeInventory && Object.keys(p.sizeInventory).length
      ? sizes.reduce((sum, size) => sum + getSizeStock(p, size), 0)
      : Number(p.stock) || 0;
        ...
        <div class="card-media-frame">
          <a href="../products/product.html?id=${p._id}" class="card-media-link">
            <div class="card-media">
              <img class="img-a" src="${esc(img1)}" alt="${esc(p.name)}" loading="lazy" onerror="handleProductImageError(this)">
  ...
  grid.querySelectorAll('.add-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!p) return;
      const sizes = (p.sizes && p.sizes.length) ? p.sizes : ['S', 'M', 'L', 'XL'];
      const size = sizes.find(candidate => getSizeStock(p, candidate) > 0);
      if (!size) return;
      addToCart(pid, size);
