...
function imgUrl(filename) {
  return `${IMG}/uploads/products/${filename}`;
}
      ...
      <div class="card-media-frame">
        <a href="products/product.html?id=${p._id}" class="card-media-link">
          <div class="card-media">
            <img class="img-a" src="${img1}" alt="${p.name} | ${color} ${typeLabel}" loading="lazy">
            <img class="img-b" src="${img2}" alt="" loading="lazy">
            <span class="hangtag">${fmt(p.price)}</span>
            <span class="swatch-dot" style="background:${swatchBg}" title="${color}"></span>
            ${stockBadge}
          </div>
        </a>
        <button class="wishlist-heart" data-wishlist-id="${p._id}" aria-label="Add to wishlist">♡</button>
      </div>
      <div class="card-info">
        <a href="products/product.html?id=${p._id}" class="pd-related-name-link"><h3>${p.name}</h3></a>
        <div class="meta">${color} · ${typeLabel}</div>
        <div class="card-variant-info">
          <div class="variant-row"><span class="variant-label">Sizes</span><span class="variant-value">${sizesDisplay}</span></div>
          <div class="variant-row"><span class="variant-label">Colors</span><span class="variant-value">${colorsDisplay}</span></div>
        </div>
        <a href="products/product.html?id=${p._id}" class="view-product-btn">View Product →</a>
      </div>`;
