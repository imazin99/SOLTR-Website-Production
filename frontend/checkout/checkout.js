...
═══════════════════════════════════════════════════ */
const CO = {
  /** Display fallback only; the server derives the authoritative shipping fee. */
  SHIPPING_FEE : 0,
...
/** Saved addresses loaded from the authenticated customer address API. */
let _savedAddresses = [];
 ...
 * @returns {string}
 */
const imgUrl = filename => window.productImageUrl(filename) || CO.FALLBACK_IMG;
...
function renderTotals() {
  ...
  if (shippingEl) {
    shippingEl.innerHTML = CO.SHIPPING_FEE > 0
      ? fmt(CO.SHIPPING_FEE)
      : '<span class="co-shipping-message">🇪🇬 We Ship Across Egypt</span>';
  }
...
/* ═══════════════════════════════════════════════════
   ...
   Reuses /api/customers/me/addresses. The backend derives the customer
   from the verified JWT and scopes every address operation server-side.
═══════════════════════════════════════════════════ */
...
function setCheckoutField(id, value) {
  const input = $el(id);
...
function getCheckoutAddressPayload() {
  return {
    fullName: $el('co-name')?.value.trim() || '',
...
function selectedAddress() {
  return _savedAddresses.find(address => String(address._id) === String(_selectedAddressId)) || null;
}

function updateAddressSaveControls() {
  const saveCheckbox = $el('co-save-address');
  const controls = $el('co-address-save-controls');
  const mode = $el('co-address-save-mode')?.value || 'new';
  const targetLabel = $el('co-address-update-target-label');
  const target = $el('co-address-update-target');

  if (controls) controls.hidden = !saveCheckbox?.checked;
  const showTarget = Boolean(saveCheckbox?.checked && mode === 'update' && _savedAddresses.length);
  if (targetLabel) targetLabel.hidden = !showTarget;
  if (target) target.hidden = !showTarget;
  if (target && showTarget && !target.value) {
    target.value = _selectedAddressId || String(_savedAddresses[0]?._id || '');
  }
}

function renderAddressUpdateOptions() {
  const target = $el('co-address-update-target');
  if (!target) return;
  target.innerHTML = _savedAddresses.map(address => `
    <option value="${esc(address._id)}">${esc(address.label || 'Address')} — ${esc(address.city || '')}</option>
  `).join('');
  target.value = _selectedAddressId || String(_savedAddresses[0]?._id || '');
  updateAddressSaveControls();
}

function applySavedAddress(address) {
  if (!address) return;
  _selectedAddressId = String(address._id);
  setCheckoutField('co-name', address.fullName);
  setCheckoutField('co-phone', address.phone);
  setCheckoutField('co-city', address.city);
  setCheckoutField('co-address', address.address);
  const label = $el('co-address-label');
  if (label) label.value = address.label || 'Home';
  const radio = document.querySelector(`input[name="savedAddress"][value="${CSS.escape(String(address._id))}"]`);
  if (radio) radio.checked = true;
  const updateTarget = $el('co-address-update-target');
  if (updateTarget) updateTarget.value = String(address._id);
}
...
function clearSavedAddressSelection() {
  ...
  setCheckoutField('co-name', '');
  setCheckoutField('co-phone', '');
  setCheckoutField('co-city', '');
  setCheckoutField('co-address', '');
  const label = $el('co-address-label');
...
function renderSavedAddresses() {
  const book = $el('co-address-book');
  const cards = $el('co-address-cards');
  const saveWrap = $el('co-address-save-wrap');
  if (!book || !cards || !saveWrap) return;
  ...
  book.hidden = false;
  saveWrap.hidden = false;
  cards.innerHTML = _savedAddresses.map(address => {
    const id = String(address._id);
    return `
      <label class="co-address-card" data-address-id="${esc(id)}">
        <input class="co-address-radio" type="radio" name="savedAddress" value="${esc(id)}">
        <span class="co-address-card-body">
          <span class="co-address-card-head">
            <span class="co-address-card-label">${esc(address.label || 'Address')}</span>
            ${address.isDefault ? '<span class="co-address-default">Default</span>' : ''}
          </span>
          <span class="co-address-card-line">${esc(address.fullName)}</span>
          <span class="co-address-card-line">${esc(address.phone)}</span>
          <span class="co-address-card-line">${esc(address.city)} · ${esc(address.address)}</span>
        </span>
      </label>`;
  }).join('');

  cards.querySelectorAll('input[name="savedAddress"]').forEach(radio => {
    radio.addEventListener('change', () => {
      const address = _savedAddresses.find(item => String(item._id) === String(radio.value));
      applySavedAddress(address);
    });
  });

  const preferred = _savedAddresses.find(address => address.isDefault) || _savedAddresses[0];
  applySavedAddress(preferred);
  renderAddressUpdateOptions();
}

async function loadSavedAddresses() {
  const headers = customerApiHeaders();
  const book = $el('co-address-book');
  const saveWrap = $el('co-address-save-wrap');
  if (!headers) {
  ...
  try {
    const response = await fetch(`${CO.API}/customers/me/addresses`, { headers });
    if (!response.ok) throw new Error('Could not load saved addresses');
    const data = await response.json();
    _savedAddresses = Array.isArray(data) ? data : [];
    renderSavedAddresses();
  } catch (error) {
    console.warn('[checkout] Saved addresses unavailable:', error.message);
    _savedAddresses = [];
    if (book) book.hidden = true;
...
function bindAddressControls() {
  $el('co-add-address-btn')?.addEventListener('click', clearSavedAddressSelection);
  $el('co-save-address')?.addEventListener('change', updateAddressSaveControls);
  $el('co-address-save-mode')?.addEventListener('change', updateAddressSaveControls);
  $el('co-address-update-target')?.addEventListener('change', event => {
    const address = _savedAddresses.find(item => String(item._id) === String(event.target.value));
    if (address) applySavedAddress(address);
  });
  renderAddressUpdateOptions();
}

async function persistCheckoutAddress() {
  const saveCheckbox = $el('co-save-address');
  const mode = $el('co-address-save-mode')?.value || 'new';
  const headers = customerApiHeaders();
  if (!saveCheckbox?.checked || mode === 'none' || !headers) return;

  const payload = {
    ...getCheckoutAddressPayload(),
    label: $el('co-address-label')?.value.trim() || 'Home',
    isDefault: Boolean($el('co-save-default')?.checked),
  };
  const addressId = $el('co-address-update-target')?.value || _selectedAddressId;
  const endpoint = mode === 'update' && addressId
    ? `${CO.API}/customers/me/addresses/${encodeURIComponent(addressId)}`
    : `${CO.API}/customers/me/addresses`;
  const method = mode === 'update' && addressId ? 'PUT' : 'POST';
...
function getCustomerToken() {
  try {
    return sessionStorage.getItem('soltr_customer_token')
      || localStorage.getItem('soltr_customer_token')
      || '';
  } catch {
    return '';
  }
...
async function placeOrder() {
  ...
  try {
    ...
    try {
      ...
      /* The order is already persisted; do not turn a successful order into a failed checkout. */
      console.warn('[checkout] Order placed but address was not saved:', addressError.message);
    }
    clearCartStorage();
...
async function init() {
  ...
  bindAddressControls();  /* saved address selection/save/update        */
