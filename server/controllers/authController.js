...
exports.login = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { adminId: admin._id, username: admin.username, tokenVersion: Number(admin.tokenVersion || 0) },
...
exports.logout = async (req, res) => {
  try {
    if (req.admin?.adminId) {
      await Admin.findByIdAndUpdate(req.admin.adminId, { $inc: { tokenVersion: 1 } });
    }
...
exports.me = async (req, res) => {
  try {
    ...
    console.error('[authController] Failed to fetch current admin:', err);
